...
const Fuel3 = () => {
    ...
    const [charcoalAmount, setCharcoalAmount] = useState('');
    ...
    const handleNumericInput = (text) => {
        if (/^\d*\.?\d*$/.test(text)) {
            setCharcoalAmount(text);
        }
    };

    const handleContinue = () => {
        if (!charcoalAmount || isNaN(parseFloat(charcoalAmount)) || parseFloat(charcoalAmount) <= 0) {
            alert('Please enter a valid charcoal consumption in kg.');
            return;
        }
        updateCarbonData('charcoal', parseFloat(charcoalAmount));
        navigate('/CCF-Calculator/travel');
    };
        ...
        <div className="ccf-individual-screen-container">
            ...
            <div className="ccf-individual-screen-content">
                <h2 className="ccf-individual-question-text" data-aos="fade-up" data-aos-duration="800">
                    What's the total charcoal consumption per year?
                </h2>
                <input
                    type="text"
                    className="ccf-individual-input"
                    placeholder="in kg"
                    value={charcoalAmount}
