...
const Travel7 = () => {
    ...
    const [electricCabTravel, setElectricCabTravel] = useState('');
    ...
    const handleNumericInput = (text) => {
        if (/^\d*\.?\d*$/.test(text)) {
            setElectricCabTravel(text);
        }
    };

    const handleContinue = () => {
        if (!electricCabTravel || isNaN(parseFloat(electricCabTravel)) || parseFloat(electricCabTravel) <= 0) {
            alert('Please enter a valid electric taxi travel distance in kms.');
            return;
        }
        updateCarbonData('electricCabTravel', parseFloat(electricCabTravel));
        navigate('/CCF-Calculator/travel8');
    };
