export const calculateCorporateCarbonFootprint = (carbonData) => {
    const EF = {
        fuelType: {
            'Petrol': 2.29 / 1000,
            'Diesel': 2.91 / 1000,
    ...
    const refrigerantEmissions = (carbonData.selectedRefrigerants || []).reduce((total, ref) => {
        ...
        if (!details || !details.quantity) return total;
        const gwp = EF.refrigerants[ref] || EF.refrigerants['Other'];
    ...
    const electricityEmissions = normalizeMetric(carbonData.enterpriseElectricityCOnsumption || 0, carbonData.fuElMetric || '') * EF.purchasedEnergy;
    const heatEmissions = normalizeMetric(carbonData.acquiredHeat || 0, carbonData.EnergyMetric || '') * EF.purchasedEnergy;
    const steamEmissions = normalizeMetric(carbonData.acquiredSteam || 0, carbonData.steamMetric || '') * EF.purchasedEnergy;
    const coolingEmissions = normalizeMetric(carbonData.acquiredCooling || 0, carbonData.coolingMetric || '') * EF.purchasedEnergy;

    const onsiteRenewableEmissions = carbonData.generatesRewnewable
        ? normalizeMetric(carbonData.capacityForRenewable || 0, carbonData.metricForRenewable || '') * EF.renewableEnergy
        : 0;
    ...
    const transportEmissions = (carbonData.keyTransportation || []).reduce((total, mode) => {
        const ef = EF.transportModes[mode] || 0;
        return total + ((carbonData.estimatedEmissionsTransportation || 0) * ef);
    }, 0);
    ...
    const employeeCommuteEmissions = (carbonData.primaryComumute || []).reduce((total, type) => {
        ...
        const ef = EF.employeeCommute[type] || 0;
    ...
    const businessTravelEmissions =
        ((carbonData.businessTravel?.flights || 0) * EF.businessTravel.flights) +
        ((carbonData.businessTravel?.trains || 0) * EF.businessTravel.trains) +
        ((carbonData.businessTravel?.cars || 0) * EF.businessTravel.cars) +
        ((carbonData.businessTravel?.cruiseShips || 0) * EF.businessTravel.cruiseShips);

    const wasteEmissions = normalizeMetric(carbonData.recycledWaste || 0, 'Metric Tonnes') * 0.05;
