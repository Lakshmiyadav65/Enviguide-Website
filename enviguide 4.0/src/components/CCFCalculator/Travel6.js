...
const Travel6 = () => {
    ...
    const [electricBusTravel, setElectricBusTravel] = useState('');
    ...
    const handleNumericInput = (text) => {
        if (/^\d*\.?\d*$/.test(text)) {
            setElectricBusTravel(text);
        }
    };

    const handleContinue = () => {
        if (!electricBusTravel || isNaN(parseFloat(electricBusTravel)) || parseFloat(electricBusTravel) <= 0) {
            alert('Please enter a valid electric bus travel distance in kms.');
            return;
        }
        updateCarbonData('electricBusTravel', parseFloat(electricBusTravel));
        navigate('/CCF-Calculator/travel7');
    };
